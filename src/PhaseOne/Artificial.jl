...
"""
function reformulate(lp::MatOI.LPSolverForm)
    lp_nrows = nrows(lp)
    ...
    #     x = Array{Float64}(undef, ncols(lp))
    # end
    x = zeros(Float64, ncols(lp))
    ...
    # return MatOI.LPSolverForm(c, xl, xu, A, Array(lp.b), Array(lp.x), lp.TArray)
    senses = fill(MatOI.EQUAL_TO, nrows(lp))
    return MatOI.LPSolverForm{Float64, typeof(A)}(
        MOI.MIN_SENSE,
